...
class ScaledDotProductAttention(nn.Cell):
    ...
    def forward(self, q, k, v, mask=None):
        attn = torch.ops.BatchMatMul(q, k.transpose(1, 2))
        attn = attn / self.temperature
        if mask is not None:
            attn = attn.masked_fill(mask, -1e9)
        attn = self.softmax(attn)     
        attn = self.dropout(attn)
        output = torch.ops.BatchMatMul(attn, v)
        return output, attn
...
class MultiHeadAttention(nn.Cell):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1):
        ...
        self.w_qs.weight = Parameter(initializer(Normal(sigma=np.sqrt(2.0 / (d_model + d_k)), mean=0), [d_model, n_head * d_k], mindspore.float32), name = "w_qs.weight")
        self.w_ks.weight = Parameter(initializer(Normal(sigma=np.sqrt(2.0 / (d_model + d_k)), mean=0), [d_model, n_head * d_k], mindspore.float32), name = "w_ks.weight")
        self.w_vs.weight = Parameter(initializer(Normal(sigma=np.sqrt(2.0 / (d_model + d_v)), mean=0), [d_model, n_head * d_v], mindspore.float32), name = "w_vs.weight")
        
        
        self.attention = ScaledDotProductAttention(temperature=np.power(d_k, 0.5))
        self.layer_norm = nn.LayerNorm((d_model,))# 不用改
        self.fc = nn.Dense(n_head * d_v, d_model)
        mindspore.common.initializer.XavierUniform(self.fc.weight)
...
class PositionwiseFeedForward(nn.Cell):# gai!
    ...
    def __init__(self, d_in, d_hid, dropout=0.1):
        ...
        self.layer_norm = nn.LayerNorm((d_in,))# 不用改
        self.dropout = nn.Dropout(keep_prob=1 - dropout)# 不用改
...
class Transforme_Encoder(nn.Cell):
    def __init__(
...
class Transforme_Encoder_light(nn.Cell):
    def __init__(
            self, d_word_vec=512, n_layers=2, n_head=8, d_k=64, d_v=64,
...
class PP_layer(nn.Cell):#gai!
    ...
    def forward(self, enc_output):
        reading_order = mindspore.arange(self.character_len, dtype=mindspore.int64, device=enc_output.device)
        ...
        g_output = mindspore.ops.BatchMatMul(t, enc_output)  # b,25,512
        return g_output, t
