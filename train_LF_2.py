...
def flatten_label(target):
    ...
    label_flatten = ms.Tensor(label_flatten, dtype=ms.int64)
    # label_flatten = torch.LongTensor(label_flatten)
    label_length = ms.Tensor(label_length, dtype=ms.int32)
...
def _flatten(sources, lengths):
    ...
    cast_op = ms.ops.Cast()
    output = concat_op((cast_op(t[:l] for t, ms.float32), l in zip(sources, lengths)))# 照改有报错
    return output
...
if __name__ == '__main__':
    ...
    criterion_CE = nn.SoftmaxCrossEntropyWithLogits(sparse=True, reduction='mean')
    # criterion_CE = nn.CrossEntropyLoss().cuda()
    L1_loss = nn.L1Loss(reduction='mean')# 还有'sum'和'none'
